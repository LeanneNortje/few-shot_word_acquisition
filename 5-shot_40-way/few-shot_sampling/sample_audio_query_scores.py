...
ss_save_fn = '../support_set/support_set_40_classes.npz'
...
top_N = 600
temp_save = Path('audio_files')
temp_save.mkdir(parents=True, exist_ok=True)

for q_word in s_audio:
    # if q_word not in ['kite', 'dog']: continue
    newly_labeled = []
    id = key[q_word]
    if (temp_save / Path(str(id) + '.npz')).is_file(): 
        print(f'{q_word}({id}): Done.')
        continue
    query_scores = {'values': [], 'wavs': []}
    record = {}
            ...
            for x in s_audio[q_word]:
                ...
                if len(indexes) != 0:
                    ...
                    if norm_score > max_score: 
                        ...
                        record[wav] = (path, start, end, p, q, indexes)
            query_scores['values'].append(max_score)
            query_scores['wavs'].append(wav)    

    indices = np.argsort(query_scores['values'])[::-1]
    i = 0
    while i < top_N:

        wav = Path(query_scores['wavs'][indices[i]])
        wav_name = wav.stem
        ...
        path, start, end, p, q, indexes = record[str(wav)]
        _, b0 = path[start - 1]
        _, bT = path[end]
        ...
        if frames == aud.size(1):
            ...
            newly_labeled.append(wav)
        i += 1
    np.savez_compressed(
        temp_save / Path(str(id)),
        wavs=newly_labeled
        )
    # if id == 1: break
...
for q_word in vocab:
    id = key[q_word]
    newly_labeled[id] = np.load(temp_save / Path(str(id) + '.npz'), allow_pickle=True)['wavs']
